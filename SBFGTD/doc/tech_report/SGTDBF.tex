...
\begin{enumerate}
 ...
 \item For each node that needs to be handled, either queue their parent(s) for handling (in case the node was the last node in the production) or have them queue the `next' node in the production they are a part of for expansion. Repeat this process until there are no nodes left to handle.
