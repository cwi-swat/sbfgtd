...
Note that the {\bf toReduceStore} needs to be implemented as an array or table with O(1) look-up time; otherwise it is not possible to guarantee worst-case cubic time complexity. Additionally it needs to be able to contain the same number of entries as there are levels in the input string plus one. Allocation can be done before parsing starts.
