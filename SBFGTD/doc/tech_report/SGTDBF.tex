...
$S\,::=\,SS\,|\,a\,|\,\epsilon$\\
Input = $a$\\
Imagine we matched the first part of the tree and are at level $1$ now. We are trying to recognize $.SS$ and $S.S$, where $S.S$ has a prefix that starts at $0$. This means $.SS$ only has edges that point to nodes in level $1$ and $S.S$ only has edges that point to nodes in level $0$. The problem is that the order in which we handle these two nodes will determine whether or not we recognize all derivations. If we would move from $S.S$ to $SS.$ first and follow all the associated edges to level $0$, we would miss the reductions to level $1$, since the edges from $.SS$ are will not have been added to $S.S$ before the $SS.$ node is handled. On the other hand, if we would move from $.SS$ to $S.S$ first, the stacks would merge before any reductions for this production are executed. In this case $S.S$ would contain edges to both the nodes in level $0$ as the ones in $1$ before we move to $SS.$, which does lead to a correct result.

This order dependency is something we cannot enforce for certain grammars, so we need another solution for this problem. We solved this by propagating edges forward through the production. If we are currently handling a node associated with nullable derivations and are moving to the next node in the production we need to check whether or not this next node is handled yet and if it is associated with nullable derivations as well. If this is the case, we tranfer all edges that are absent on the next node to it. If missing edges where transfered, we need to continue their propagation to the node beyond this next node and so on until the last node in the production is reached. For each edge that is tranfered to the last node in the production a reduction needs to be performed. This way we can be sure all derivations will be found.

Also note that, since this propagation only happens within a single level and does not cause extra edge visits it does not influence worst-case behaviour. Furthermore, the performance impact of this solution will be very low in general; as good as non-existent in cases without hidden right recursion, since the only extra work is one cheap constant time check for each node visit. In cases where hidden right recursion is present there is no real extra work either, as all stack merges would have happened anyway. The only difference is that the same amount of work possibly gets distributed over multiple visits of the same node.
...
\subsubsection{Hidden right recursion}
As mentioned before hidden right recursion needs some extra attention. However since the recognizer produces all derivations we do not run into any real issues here. We just need to ensure we also propagate the prefixes along with the edges, as discussed in section \ref{subsec:hiddenRightRecursion}.
