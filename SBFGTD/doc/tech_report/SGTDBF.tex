...
As expected JavaCup performs best. For this specific grammar, the JavaCup parser consistently recognizes the given input between one and a half and two times as fast as ours. This seems like a big difference, however this is a fairly impressive result. Especially considering we are using a graph, since we need to be fully general, instead of the application's stack. When looking at the results for both of the SGLR version it becomes even more apparent how well we do in comparison.

Apart from this, our parser scales perfectly linear for this LR grammar, like the others. The data in the graph supports this, as the relative performance difference between them does not change for different input lengths.
