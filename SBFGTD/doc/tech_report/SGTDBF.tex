...
Reducing consumes $O(N^3)$ time in the worst-case as well. Each of the $O(N)$ edges is associated with $N^2$ reductions. The check whether or not each of the parents still needs to be reduced, is an operation that completes in $O(1)$ time. From this we can conclude that the number of edge visits falls within the $O(N^3)$ bound.
...
The reduce code need to be modified more extensively. Basically two things need to be added.
