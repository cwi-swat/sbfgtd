...
As mentioned before, distinguishing between different type of graph nodes can open up interesting opportunities for optimization.

Another of these optimizations is conditional edge sharing. The guarantee that the set of edges on a leaf node in the graph will always be identical to the set of edges on the `next' item in the production (if present), can be used to increase the level of edge sharing. The reason this is possible is because leaf nodes always have a static length. This property makes it impossible for a stack merge to occur on any node that is located directly after a leaf node in a production. For this reason the set of edges on a leaf node can be shared with its immediate right neighbour in the production. The only exception that needs to be taken into account is related to empty leaf nodes (epsilons), as sharing their edges set may cause interference with the hidden right recursion fix. While epsilons are not intended to be located in the middle of production, which makes this is a non-issue in general, there are scenarios imaginable where this may be desirable and thus we need to take the possibility of this occurring into account.
...
This grammar contains a lot of recursion and is highly ambiguous; the input, on the other hand, can not be parsed in more then one way. Regardless, one might expect non-linear performance when parsing a string for this grammar. However, as can be seen in the graph below, this is not the case for our algorithm. In this benchmark, performance seems to scale perfectly linear regardless of how the grammar is factored.
...
The non-factored prefix-shared and the left-factored versions of the grammar perform a lot better and are fairly close together in terms of performance. That the parser performs slightly better on the prefix-shared version of the grammar in comparison to the left-factored version, is purely a result of the transformation that was required to achieve the left-factoring.

Surprising though is that, while this benchmark is one of the cases that is most heavily biased in favor of look-ahead filtering, the impact it has is hardly noticeable. Disabling look-ahead filtering should have a negative influence on performance while parsing an input string for the left-factored version of the grammar, although the difference is surprisingly minor. The main reason for this is the eager matching optimization, which achieves what look-ahead filtering partially would have in this particular case. However, there are other situations in which look-ahead filtering can still be a potentially useful performance increasing tool.
...
It is worth mentioning that it is still possible to optimize the performance of the parser on LL and (LA)LR grammars. One could imagine implementing a faster path for productions in slices of the grammar that conform to certain restrictions (i.e. they only contain non-terminals that are in LR(k) for example). By doing this we would be able to match the speed of any parser.
...
\subsection{Realistic cases (NOTE: update, the measurements\\were done pre-optimization)}
