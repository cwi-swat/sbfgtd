...
Surprising though is that, while this benchmark is one of the cases that is most heavily biased in favor of look-ahead filtering, the impact it has is hardly noticeable. Disabling it should have a negative influence on performance while parsing an input string for the left-factored version of the grammar, although it is so minor that it does not seem to be worth it. The main reason for this is the eager matching optimization, which basically does what look-ahead filtering partially would have done. The trade-off here is between potentially unnecessary loop iterations and potentially unnecessary condition checks; in either case the `wasted' CPU cycles will be minimal.
...
\subsection{Realistic cases (NOTE: update, measurements are pre-optimization)}
