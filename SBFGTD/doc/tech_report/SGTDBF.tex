...
\begin{itemize}
 ...
 \item Deterministic on LL and LR grammars.
 ...
 \item Generating understandable error reports is trivial.
\end{itemize}
...
Another questions that some people will have on their mind is: ``does it terminate?". Ofcourse it does. The reason for this fairly obvious and can be even explained in one sentence; since we move forward to the next level once all work is done in the current level and we share all graph nodes within each level (of which there are a finite amount), causing the expansion and reduction phases to converge to a point where no more work can be done in a level, we reach the end of the input string at some point (assuming the input string has a finite length as well).
...
Since the mapping between the original grammar and the code or table is one-on-one, it is possible to implement or edit it by hand without much effort. In fact handcrafting it is about as simple as writting the grammar itself (although more cumbersome).
...
Reducing consumes $O(N^3)$ time in the worst-case as well. Every edge is associated with one reduction per level per graph node at most. Each of these edges can be associated with $O(N)$ nodes per level; one per level before the current level. There are $N$ levels, so each edge will be visited $O(N^2)$ times. Since there are $O(N)$ edges at most, we can conclude that the number of visited edges falls within the $O(N^3)$ bound. Each of the operations executed during an edge visit completes in constant time, so the worst-case time bound of the reducer is directly related to the number of edge visits.
...
Keeping worst-case behaviour in check is great, but is generally less important then having good performance and scalability in more common cases. Here we will prove that our algorithm is linear, not only for LL and LR grammars, but for all unambiguous context-free grammars.

The reason for this is fairly simple. In our algorithm a stack merge or edge that is visited twice only occurs in cases where an ambiguity is encountered. The cause of these events is always that a certain portion of the input string was derived in more then one way. When parsing an input string for an unambiguous language neither of these events can ever occur. The result of this is that every graph node can only have one edge and one unique graph node that preceded it. While the stack can split, it will never result in more then one node per non-terminal sort per level. Combined with the linear time graph expansion guarantee discussed in section \ref{subsec:nodeExpansionOptimization}, this means we can recognize every possible input string for an unambiguous grammar in $O(N)$ time.

Furthermore, performance degradation with ambiguous grammars will be gracefull. Meaning input strings for near unambiguous grammar will be parsed in near $O(N)$ time.
...
Note however that there is a strong relation between the parser's internal graph representation and the resulting parse forest; the parse forest is in fact an exact representation of what the parser recognized. Because of the relation, the resulting parse forest can contain cycles.
...
Each grammar rule can have a number of actions associated with it. These actions can be used to filter alternatives that cannot be disambiguated using other filters or disambiguate non-context-free ambiguities (like C typedefs). We will not go into detail about this feature, since actions are outside of the scope of this article. The option exists to either intergrate the execution of actions into the parser or to apply them as a post-parse filter during, or after, flattening; depending on the requirements or preference of the user. However their implementation is not trivial, because of their interaction with ambiguities and the parse forest.
...
Note however that the recognizer is optimized for speed and the parser for a balance between memory usage and speed (meaning a faster implementation is possible at the cost of increased memory footprint).
...
What may appear to be surprising is the limited performance difference between both cases. Even though the version with the $\epsilon$ yields a more complicated tree and nullables need some special care, the actual extra work involved in handling them is almost negligable. This is in line with what we asserted in section \ref{subsec:hiddenRightRecursion}.
...
Note how these results also support what we asserted in section \ref{subsec:unambiguousTimeComplexity}.
...
\begin{itemize}
 ...
 \item Matching could be handled more efficiently, so we `touch' less terminals.
...
-As scalable as other cubic parsers (this is as good as one can get).\\
-Linear on unambiguous grammars (which is better then everything else?).\\
-Faster then GLR (a lot less edge visits / graph searching. (Missing text to support this atm)).\\
-As simple as Earley.
