...
As expected JavaCup performs better. For this specific grammar, JavaCup consistently parses the given input about twice as fast. This seems like a big difference, however this is a fairly reasonable result. Especially considering we are using a graph, since we need to be fully general, instead of the application's stack. Apart from this, our parser scales exactly the same on this LR grammar (linear). The data in the graph supports this, as the relative performance difference between the two does not change for different input lengths.
