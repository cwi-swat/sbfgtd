...
As is common in general parsing, ultimately you often end up with an ambiguous parse forest. In this chapter we will discuss a number of features that can be used to filter the trees in this forest, in case this is required. These features are all optional and have no dependencies on eachother.

\subsection{Restrictions and requirements}

First of all one could add restrictions to grammar items. For example, marking a certain list item with a restriction that prevents it from being followed by a certain set of characters or strings, could be used to indicate eagerness for this list item. Another example would be to require a certain item to start at the beginning of a line or at a certain column. One can think of numereous different possibilities; basically any restriction or requirement on the substring an item matched or the input preceding or following an item is imaginable. These restrictions and requirements include, but are not limited to, sets of characters, string and regular expressions.

When adding this feature to the implementation of the recognizer / parser one can make the destinction between filters that need to be executed before reducing and filters that can also be executed before expanding an item, for efficiency reasons. Filters that can be executed before the expansion of an item are almost exclusively limited to restrictions and requirements that deal with input that preceded an item, all other filters can only be handled before the reduction of their associated item, since we generally do not have all the required information to handle these before that time. When a filter succesfully matches it should prevent either the expansion or reduction of the item it is associated with, to prevent the recognizer from recognizing it and prevent the possible results this item would have had from being added to the parse forest.

\pagebreak
\subsection{Priorities and associativity}

Priority and associativity relations are another type of filter which can be a useful for disambiguation. Priority and associativity relations can be implemented as nesting restrictions. For example, if we take the grammar rule $E\,::=\,E\,*\,E\,>\,E\,+\,E$, to indicated that $E\,*\,E$ has a higher priority then $E\,+\,E$, this means $E\,+\,E$ cannot be nested on either side of the $E\,*\,E$ alternative. Associativity works in a similar way; $E\,::=\,E\,+\,E\,\{left\}$ means that it is not allowed for $E\,+\,E$ to be a child of itself in the right most $E$ of this alternative. The same goes for right associative rules, but the other way around. Making a rule non-associative is also possible, this will prevent it from being nested on either side of itself.
