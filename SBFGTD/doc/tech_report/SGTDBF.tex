...
The result nodes form the links between the prefix sets and result stores. There are a cubic amount of these in the worst-case, as they can be identified by the start location of their prefix set, the end location of their result store and the combination of the end location of their prefix set an start location of their result store, as these needs to be the same. So there are three identifying numbers, representing locations in the input string, which make each result node unique, thus there can never be more then $O(N^3)$ of them. This is reflected by the numbers in the table.
