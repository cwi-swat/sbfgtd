...
Keeping worst-case behaviour in check is great, but is generally less important then having good performance and scalability in more common cases. Here we will prove that our algorithm is linear, not only for deterministic (such as LL and LR) grammars, but for all unambiguous context-free grammars.
...
Furthermore, performance degradation with ambiguous grammars will be graceful. Meaning input strings for near unambiguous grammar will be parsed in near $O(N)$ time.
