...
The reason for this is fairly simple. In our algorithm a stack merge or edge that is visited twice only occurs in cases where an ambiguity occurs. The cause of these events is always that a certain portion of the input string was derived in more then one way. When parsing an input string for an unambiguous language neither of these events can ever occur. The result of this is that every graph node can only have one edge and one unique graph node that preceded it. While the stack can split, it will never result in more then one node per non-terminal sort per level at most. Combined with the linear time graph expansion guarantee described in section \ref{subsec:nodeExpansionOptimization}, this means we can recognized every possible input string for an unambiguous grammar in $O(N)$ time.

Furthermore, performance degradation will be gracefull. Meaning input strings for near unambiguous grammar will be parsed in near $O(N)$ time.
