...
Proving something correct is complicated, so I'm not going to attempt that. Instead I'll go into detail on some special cases that deserve some attention.
...
$S ::= A$\\
$A ::= Aa | a$\\
We'd end up with the following GSS:
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{left-recursive.png}
\caption{GSS representation of a left-recursive grammar.}
\end{figure}
