...
This is the reduce loop of the recognizer. We take one node from the {\bf toReduceSet} and put it in the {\bf reducedStore} to indicate it should not be reduced again; each node is only allowed to get reduced once. In case the node we are reducing is a terminal, we need to check whether or not it matches the substring in the input it is associated with; if this is not the case we discard the node.

After this two things can happen, either the node we are reducing is the last node in a production or it isn't. In case it is, we follow it's edges and queue the nodes that have not been reduced yet to the {\bf toReduceSet}. If the node isn't the last node in the production it is a part of, we queue it's 'right neighbour' in the production for expansion, if not done so already. ............ update edges .....

TODO: Fix edge problem in this non-breadth first version.
