    ...
    if(node.lastInProduction()){
      ...
      for(parent <- node.edges){
        if(toReduceSet.notContains(parent) &&
            reducedStore.notContains(parent)){ // Sharing
      ...
      if(expandedStore.notContains(next)){ // Sharing
        next = expandedStore.get(next);
      }else{
...
reduce()\{
  while(node <- toReduceSet)\{
    if(node.lastInProduction())\{\textit{
      if(resultStores.contains(node.edges.getOne()))\{
        resultStore = resultStores.get(node.edges.getOne());
      \}else\{
        resultStore = createResultStore(node.edges.getOne());
