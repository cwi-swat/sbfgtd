...
reduce(){
  while(node <- toReduceSet){
    if(node.lastInProduction()){
      if(reducedStore.notContains(node)){
        for(parent <- node.edges){
          if(toReduceSet.notContains(parent)){ // Sharing (reduce check)
...
Note that the {\bf reducedStore} needs to be implemented as an array or table that has a set of non-terminals associated with every index in the input string (with O(1) look-up time for the array / table). Otherwise it is not possible to guarantee worst-case cubic time complexity.

TODO: Explain reducedStore a bit better.
...
reduce()\{
  while(node <- toReduceSet)\{
    if(node.lastInProduction())\{\textit{
      if(resultStore.contains(node.edges.getOne()))\{
        result = resultStore.get(node.edges.getOne());
      \}else\{
        result = createResult(node.edges.getOne());
