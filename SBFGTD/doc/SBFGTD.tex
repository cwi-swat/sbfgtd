...
This means that the algorithm remains within cubic time bounds in the worst-case scenarios.
...
\begin{tabular}{ | p{7ex} | p{7ex} | p{7ex} | p{10ex} | p{8ex} | p{10ex} | p{10ex} | }
  ...
  \hline
\end{tabular}
\caption{Worst-case parse tree statistics}
\end{table}
...
In this article we discribed our parsing algorithm, discussed optimizations that can be applied to it and gave an impression of its capabilities. To summarize, we developed a parsing algorithm that is both easy to comprehend and scales well, regardless of the input or grammar used. The best of both worlds, combined into one beautiful, elegant solution.
