...
In this article we will describe our general top down parsing algorithm. Our intent with this algorithm was to develop something that is both easy to comprehend and performs well on any grammar. The user should not be required to refactor a grammar to enable the parser to complete within an acceptable amout of time. Additionally, it should not be hard to grasp what is going on under the hood, nor should it be complicated to create an implementation of this algorithm.
...
First of all, left recursion. Normal top-down parsers cannot handle grammar rules containing left recursion. Because we introduce sharing into the GSS we automatically terminalize all these rules; fixing this issue. For example, if we take the grammar:\\
$S ::= A$\\
$A ::= Aa\,|\,a$\\
...
\caption{GSS representation of a expanded left-recursive grammar.}
\end{figure}
As you can see there a cycle was added to the graph, since $.Aa$ is, in a way, a child of itself. To see how left recursive grammars work in action see section \ref{sec:tracesLeftRecusive}.

Another requirement of the algorithm, of which the reason may not be immidiately obivious, is that it needs to be level synchronized. We will demonstrate why it is more complicated to get a depth-first sibling of this algorithm working correctly, by means of an example. Consider the following grammar: $S ::= SSS\,|\,a\,|\,aa$. Now imagine we are somewhere in the middle of recognizing an input string containing a number of $a$'s; currently we are recognizing $SS.S$, which matches with one $a$. The preceding items that belong to this production, $.SSS$ and $S.SS$ both recognized one $a$ as well. Causing $S(S(a),S(a),S(a))$ to be reduced at this location. We progress a bit more and are now recognizing $.SSS$ at a location that is one character earlier in the input string as the other $.SSS$ we recognized before; this time it matches $aa$. Since we now end up at a $S.SS$ node at a location where it was already recognized, we do not progress further; this node needs to be shared, otherwise we would do duplicate work, breaking the cubic time bound. However, sharing this node means we stop recognizing the current derivation, preventing reductions from being executed. Which in this case will mean that we fail to recognize the derivation $S(S(aa),S(a),S(a))$ at the current location. If we would be using a different grammar it could even cause recognition to fail entirely because of this behaviour. This problem could be solved by tracing forward through the production, gathering the missing derivation(s) and executing reductions where necessary. In our opinion this is to complicated and, above all, unnecessary. Besides that, it is also significantly slower, because of all the extra work involved. Since our algorithm is breadth-first, we will never have stacks running ahead of others, eliminating the possibility to this behaviour popping-up. We do need extra iterations within the same level to handle nullables, but this is only a problem for the parser, which needs to keep track of hidden right recursion. We will cover the handling of hidden right recursion in the parser in section \ref{sec:parserCorrectness}.
...
A more obvious optimization is adding support for look-ahead filtering. Simply by adding simple checks to the (generated) parser code, we can prevent unnecessary work; if a certain alternative will never match, we do not need to expect it. The number of characters we look ahead is not limited by any theoretical or practical constraints.
...
It is possible to resolve this problem, however this would require a second pass over the tree. Additionally it would prevent us from being able to remove parses that will be filtered in the end, before exploring them further. So it would impact performance in more then one way. This is the main reason why we dicided not to support nested reject rules.
...
It contains lots of left-recursion and is highly ambiguous; the input, on the other hand, is not ambiguous. One would expect non-linear performance when parsing a string for this grammar. However, as can be seen in the graph below this is not the case. Performance seems to scale perfectly linear regardless of how the grammar is factored.
...
-Using realistic cases would be nice (Java, C, ...?).\\
-SDF parse speed: 230K to 300K char/sec if filtering and flattening are included, 360K to 530K char/sec without filtering and flattening. (SGLR is about 1.6 times as slow in this case). (I estimate a C implementation of this parsing algorithm would get around 1M char/sec with filtering and well over 1.5M char/sec without filtering, on this particular grammar).

\section{Prototype}

To validate our algorithm we made a parser implementation in Java. In this chapter we will discuss its overall design and implementation.
...
TODO: Write.\\
1. Matching can be handled more efficiently.\\
2. Prefix sharing can be implemented more efficiently by moving more logic to the generator and modifying the implementation slightly.\\
3. We can extract information from the generated code to build and cache a table with `sort -> (alteratives + look-aheads)' mappings instead of calling methods, handling look-ahead and rebuilding stuff all the time.\\
3.1. Prefix sharing can be done in a better way; the parser can compute it while building the mentioned table.\\
3.2. Nullable trees could be pre-computed or dynamically cached in this table.\\
3.3. Priority / associativity filtering can be encoded in this table, giving a major performance boost.\\
3.4. Some rejects can be statically computed and encoded in this table as `k' look-ahead things which dynamically prevent queueing for certain nodes (think of rejected keywords and such).

NOTE: These are paritially algorithmic and partially implementation related improvements.

\section{Conclusion}

In this article we described our parsing algorithm, discussed optimizations that can be applied to it and gave an impression of its capabilities. To summarize, we developed a parsing algorithm that is both easy to comprehend and scales well, regardless of the input or grammar used. The best of both worlds, combined into one beautiful, elegant solution.
