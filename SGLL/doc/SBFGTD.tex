...
Finally, we can also gain something at the other end. The sharing check on nodes when reducing can be eliminated. Since every GSS node of the same 'sort' always has exactly the same children if they are in the same level, it is sufficient to initially just follow the first edge in its edge set. In case the node this edge points to has already been reduced in this level nothing needs to be done (except record the alternative, in case we're parsing and not just recognizing); otherwise all other edges need to be followed as normally would have been the case. This improves the performance of the parser by a factor, dependent on the definition of the rules in the grammar.
