...
expand(){
  while(node <- toExpandSet){
    if(node.isTerminal()){
      ...
      for(childNode <- getChildrenFor(node)){
        if(expandedSet.contains(childNode)){
          INode sharedChildNode = expandedSet.get(childNode);
          sharedChildNode.addEdge(node);
        }else{
  ...
  while(node <- toReduceSet)\{
    if(node.lastInProduction())\{\textit{
      ...
      \}else\{
        ...
        for(parent <- node.edges)\{
          if(toReduceSet.notContains(parent) &&\textit{
              resultStores.notContains(parent)})\{ // Sharing
            toReduceSet.add(parent);
          \}
        \}
      \}
