...
Since the GSS only contains nodes that we expect to 'encounter', there are at most O(N) of them; one of every type per location in the input string. Each of these nodes contains edges back to their 'parents'. A node only has one of these to each of it's possible parents, per level; since there are at most O(N) levels there are at most O(N) edges per node.

// TODO Convert the rest into text.


-Every node 'matches' $N$ unique substrings (start + end position).\\
-Every node has a reference to the 'next' node in the production.\\
-The recognizer is $O(N^{3})$ worst-case (TODO: Figure out why)\\
