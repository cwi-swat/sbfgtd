...
\begin{itemize}
 ...
 \item No noticable performance hit for rules that are not left factored.
...
Here we see a section of the parse forest. It illustrates how the results of the production $S\,::=\,AAAA\,|\,AAA\,|\,AA$ are stored in memory. Also note how nodes with the same start position have the same set of pointer associated with them, as indicated at the start of this chapter.
...
Related to the length of the input string, the number of nodes scales worse then $O(N^{2})$ but better then $O(N^{3})$.
...
TODO: highlight additions, so it's easy to see where the changes are.
...
-Lists, separated lists and optionals.

\subsection{Non-context free}

-Start-of-line, end-of-line and at-column 'epsilons'.
...
-Making a $O(N^2)$ recognizer or something :P? If only we could do edge merging faster, or solve it differently so it's not necessary ....
