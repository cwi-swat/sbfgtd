...
In many grammars productions exist that start with the same symbols. There is no reason why we would have do duplicate work for these symbols. For example, if we take the grammar: $S\,::=\,E\,+\,E\,|\,E\,-\,E$. Both $E$'s of the left of the productions match the same substring(s), thus these two productions may as well be merged into $S\,::=\,E\,(+\,E\,|\,-\,E)$. It is trivial to modify our algorithm to allow this. Simply by allowing every node in the GSS to have more then one 'next' node and assigning the same id to both $E$'s, the desired result is achieved. This optimization ensures we can recognize / parser LR(k) grammars in linear time.
